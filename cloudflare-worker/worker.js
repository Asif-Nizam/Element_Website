/* ============================================================
   ...
   Gemini API. It holds the secret API key (never exposed to
   visitors) and the studio's knowledge base, and answers visitor
   questions using Gemini 2.5 Flash-Lite.
   ...
   4. Go to Settings -> Variables and Secrets on the Worker ->
      "Add" -> name it GEMINI_API_KEY, paste your key from
   ...
   That's it — no billing setup beyond your Google AI Studio account
   (Cloudflare's free tier covers 100,000 requests/day; Gemini
   2.5 Flash-Lite's free tier covers roughly 1,000+ requests/day
   on its own, so this can realistically run at ₹0/month unless
   traffic gets unusually high).
   ============================================================ */
...
- For internal pages, link with an absolute path, e.g. [See all programs](/programs) or [View the gallery](/gallery).
...
Website pages you can point people to: Home (/), Programs (/programs) for training/pricing details, Gallery (/gallery) for photos and videos of past sessions and events, Team (/team), About (/about), Contact (/contact) for the map and contact form.
...
export default {
  async fetch(request, env) {
    ...
    // Gemini uses "model" instead of "assistant" for the AI's turn, and
    // wraps text in a "parts" array rather than a plain string.
